$(document).ready(function() {
    var dataTable = $('#table_data').DataTable({
        ...
        "pageLength": 50,
        "ajax": {
            url: site_url + '/droug_opd/fetch_droug_opd',
            data: {
...
crud .ajax = {
    del_data:function (id,cb){
        var url = '/droug_opd/del_droug_opd',
            params = {
                id: id
            }
        ...
        app.ajax(url, params, function (err, data) {
            err ? cb(err) : cb(null, data);
             ...
             var url = '/droug_opd/save_droug_opd',
                 params = {
                     items: items
                 }
             ...
             app.ajax(url, params, function (err, data) {
                 err ? cb(err) : cb(null, data);
                   ...
                   var url = '/droug_opd/get_droug_opd',
                       params = {
                           id: id
                       }
...
crud.set_after_update = function (items,row_id) {
    ...
    row_id.find("td:eq(0)").html(items.id);row_id.find("td:eq(1)").html(items.didstd);row_id.find("td:eq(2)").html(items.didstd_19);row_id.find("td:eq(3)").html(items.dname);row_id.find("td:eq(4)").html(items.dname_thai);row_id.find("td:eq(5)").html(items.drug_detail);row_id.find("td:eq(6)").html(items.sound);row_id.find("td:eq(7)").html(items.vdo);row_id.find("td:eq(8)").html(items.note);row_id.find("td:eq(9)").html(items.use);

}
crud.set_after_insert = function (items,id) {

            $('<tr name="row'+(id+1)+'"><td>'+id+'</td>' +
                '<td>' +items.id+'</td>' +'<td>' +items.didstd+'</td>' +'<td>' +items.didstd_19+'</td>' +'<td>' +items.dname+'</td>' +'<td>' +items.dname_thai+'</td>' +'<td>' +items.drug_detail+'</td>' +'<td>' +items.sound+'</td>' +'<td>' +items.vdo+'</td>' +'<td>' +items.note+'</td>' +'<td>' +items.use+'</td>' +
...
crud.set_update = function (data,row_id) {
    $("#row_id").val(row_id);
    $("#id").val(data.rows["id"]);$("#didstd").val(data.rows["didstd"]);$("#didstd_19").val(data.rows["didstd_19"]);$("#dname").val(data.rows["dname"]);$("#dname_thai").val(data.rows["dname_thai"]);$("#drug_detail").val(data.rows["drug_detail"]);$("#sound").val(data.rows["sound"]);$("#vdo").val(data.rows["vdo"]);$("#note").val(data.rows["note"]);$("#use").val(data.rows["use"]);
}
...
$('#btn_save').on('click', function (e) {
    ...
    items.id=$("#id").val();items.didstd=$("#didstd").val();items.didstd_19=$("#didstd_19").val();items.dname=$("#dname").val();items.dname_thai=$("#dname_thai").val();items.drug_detail=$("#drug_detail").val();items.sound=$("#sound").val();items.vdo=$("#vdo").val();items.note=$("#note").val();items.use=$("#use").val();
...
function validate(items){
    ...
    if (!items.id) { swal("กรุณาระบุรหัส");$("#id").focus();}else if (!items.didstd) { swal("กรุณาระบุรหัสยา24หลัก");$("#didstd").focus();}else if (!items.didstd_19) { swal("กรุณาระบุรหัสยา 19 หลัก");$("#didstd_19").focus();}else if (!items.dname) { swal("กรุณาระบุชื่อยา Eng");$("#dname").focus();}else if (!items.dname_thai) { swal("กรุณาระบุชื่อยาไทย");$("#dname_thai").focus();}else if (!items.drug_detail) { swal("กรุณาระบุคำอธิบาย");$("#drug_detail").focus();}else if (!items.sound) { swal("กรุณาระบุเสียง");$("#sound").focus();}else if (!items.vdo) { swal("กรุณาระบุvdo");$("#vdo").focus();}else if (!items.note) { swal("กรุณาระบุบันทึกเพิ่มเติม");$("#note").focus();}else if (!items.use) { swal("กรุณาระบุสถานะการใช้");$("#use").focus();}
    else{
        return true;
    }
